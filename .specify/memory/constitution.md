...
All implementations MUST use the following technology stack unless explicitly justified and approved through the amendment procedure.

### Backend Architecture

- **Runtime**: Node.js with TypeScript for type safety in security-critical code
- **API Framework**: Express.js with middleware-based security enforcement
- **Security Middleware**: helmet.js, cors, express-rate-limit for API protection
- **Validation**: Zod for schema validation and type-safe input validation

**Rationale**: Express.js provides proven middleware architecture for implementing security controls. TypeScript reduces bugs in security-critical code paths.

### Database & Storage

- **Primary Database**: PostgreSQL for ACID compliance, audit trail integrity, and row-level security
- **Document Storage**: PostgreSQL metadata + encrypted file system storage
- **Audit Logs**: Separate append-only PostgreSQL table with partitioning by date
- **Search**: PostgreSQL full-text search with pg_trgm and ts_vector extensions (sufficient for 10,000 documents)
...
- **Authentication**: Passport.js with local strategy for session-based auth
- **Session Management**: PostgreSQL-based sessions for simplified deployment
- **Password Hashing**: bcrypt or argon2 with appropriate cost factors
- **Encryption at Rest**: pg_crypto extension for AES-256-GCM encryption
- **Encryption in Transit**: TLS 1.3 via Caddy reverse proxy (automatic HTTPS)
- **Key Management**: Environment variables for development; upgrade to HashiCorp Vault or AWS KMS for production

**Rationale**: Passport.js is battle-tested for authentication. PostgreSQL session storage eliminates need for separate session store. pg_crypto provides cryptographic functions without external dependencies.

### API Layer

- **API Documentation**: OpenAPI 3.0 with swagger-jsdoc for auto-generation
- **Input Validation**: Zod schemas for all endpoints
- **Error Handling**: Structured error responses with appropriate HTTP status codes
- **Rate Limiting**: Per-IP and per-user rate limits to prevent abuse

**Rationale**: OpenAPI documentation is essential for ISO 27001 Control A.14 (documented interfaces). Zod provides type-safe validation that catches errors at compile time.
...
- **Linting**: ESLint with eslint-plugin-security and eslint-plugin-no-secrets
- **Formatting**: Prettier for consistent code style
- **Git Hooks**: Husky + lint-staged for pre-commit validation
- **Testing Framework**: Jest for unit/integration tests, Supertest for API tests
- **Security Testing**: npm audit, Snyk for dependency scanning

**Rationale**: Security-focused linting catches common vulnerabilities during development. Automated pre-commit hooks prevent secrets and vulnerable code from entering repository.
...
- **Process Management**: PM2 for Node.js process monitoring and zero-downtime restarts
- **Logging**: Winston or Pino for structured JSON logging with log levels

**Rationale**: Docker ensures environment consistency (ISO 27001 Control A.12). Caddy provides automatic HTTPS certificate management. PM2 enables zero-downtime deployments.
...
- **Metrics Collection**: Basic metrics via PM2 or custom Prometheus integration (optional)
...
4. **Maintainability**: TypeScript and structured architecture support long-term maintenance
...
- Use TypeScript for type safety (no plain JavaScript in production code)
- Use parameterized queries or ORM for all database access (prevent SQL injection)
- Use Zod or equivalent for all external input validation
